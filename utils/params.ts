...
const QUERY_PARAMS_TYPE_CASTERS: Record<
  QueryParamType,
  (queryValue: string) => undefined | CastedQueryParamItem
> = {
  ...
  number: (queryValue): undefined | number =>
    Number.isFinite(parseFloat(queryValue))
      ? parseFloat(queryValue)
      : undefined,
};
const QUERY_PARAMS_TYPE_STRINGIFYERS: Record<
  QueryParamType,
  (queryValue: undefined | CastedQueryParamItem) => string | undefined
> = {
  ...
  number: (queryValue: number | undefined): string | undefined =>
    typeof queryValue === 'number' ? queryValue.toString(10) : undefined,
};
  ...
  return Object.keys(definitions).reduce((castedQuery, definitionName) => {
    const values: string[] =
      typeof query[definitionName] === 'undefined'
        ...
        : (query[definitionName] as string[]);
    let castedValue: CastedQueryParam;
      ...
      if (definition.mode === 'collection') {
        addedParts = Array.from(
          new Set([
            ...
            ).map<string>((castedValue: CastedQueryParamItem): string =>
              QUERY_PARAMS_TYPE_STRINGIFYERS[definition.type](castedValue),
            ),
          ]),
        );
      } else {
          ...
          typeof actualValue !== 'undefined'
            ? [
                QUERY_PARAMS_TYPE_STRINGIFYERS[definition.type](
                  actualValue as CastedQueryParamItem,
                ),
              ]
            : [];
      }
